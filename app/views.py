...
from .forms import CustomerRegistrationForm, Customer, CustomerProfileFrom
...
def show_cart(request):
 if request.user.is_authenticated:
  ...
  if cart_product:
   ...
   return render(request,'app/addtocart.html',{'carts':cart,'totalamout':total_amount,'amount':amount})
  else:
...
def address(request):
 ...
 return render(request, 'app/address.html',{'add':add,'activie':'btn-primary'})
@login_required()
def orders(request):
...
#  if data == None:
#   men= Product.objects.filter(catagory='M')
#  elif data == 'Redmi' or data == 'Samsung':
#   men = Product.objects.filter(catagory='M').filter(brand=data)
...
class ProfileView(View):
 ...
 def post(self,request):
  ...
  if form.is_valid():
   ...
   reg.save()
   messages.success(request,'Cungratulations!! Profile Updated Successfully')
   return render(request,'app/profile.html',{'form':form,'activie':'btn-primary'})